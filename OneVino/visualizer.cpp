...
void Visualizer::drawFace(cv::Mat& img, Face::Ptr f, bool drawEmotionBar) {
    ...
    cv::putText(img,out.str(),cv::Point2f(static_cast<float>(f->_location.x), static_cast<float>(f->_location.y - 20)),cv::FONT_HERSHEY_SIMPLEX,0.6,genderColor,1,8,false);
...
void Visualizer::draw(cv::Mat img, std::list<Face::Ptr> faces) {
    ...
    for (auto&& face : faces) {
        if (emotionVisualizer) {
            ...
            //std::cout <<"framecount:"<< frameCounter << "getid :"<<face->getId()  << std::endl;
